...
if not OPENAI_API_KEY:
    raise RuntimeError('OPENAI_API_KEY が未設定です（Environment Variables を確認してください）')
# sk-proj- 系のキーは project 指定が必要なケースがあるため明示する
...
def load_stocks():
    ...
    try:
        stocks_path = os.environ.get('STOCKS_JSON_PATH') or os.path.join(BASE_DIR, 'stocks.json')
